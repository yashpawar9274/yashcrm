...
import { requireActiveAccount, requirePlanCapacity, toErrorResponse } from '@/lib/auth/account'
...
export async function POST(request: Request) {
  ...
  try {
    const ctx = await requireActiveAccount('agent')
    await requirePlanCapacity(ctx, 'automations')
  } catch (err) {
